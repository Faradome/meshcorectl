...
from ..output import render, render_result
...
@click.pass_obj
def create_contact(state: Any, uri: str, dry_run: bool) -> None:
    ...
    click.echo(render_result(result, state.output, f"contact imported from {uri}"))
...
def create_channel_command(
    state: Any, index: int, name: str, key: str | None, dry_run: bool
) -> None:
